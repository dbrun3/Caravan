...
func _enter_tree():
	...
	panel_container.add_child(_hbox_container)
	...
	for i in range(max_options):
		...
		button.rect_min_size = Vector2(180, 23)
...
func proceed(idx):
	...
	match(type):
		...
		'3':
			# signal
			var signalval = dialogue_data.nodes[idx]['signalValue']
			var signallink = dialogue_data.nodes[idx]['link']
			if signallink == 'END':
				stop()
				emit_signal('dialogue_signal', signalval)
			else:
				currentPage = signallink
				emit_signal('dialogue_signal', signalval)
				proceed(signallink)
		'4':
			# set
...
func _set_dialogue(dict):
	...
	dialogue.add_color_override("default_color", Color(0.5,1,0.5,1))
